function [rdval_1,ptgval_1,rdval_2,ptgval_2,combval,interval] = RelaTek_m(dim,t,raw_output,raw_input,srch_opts,crit_opts,win_opts,filt_opts,plot_opts,col_opts,coeff_reg,interval)
...
rdarr_1 = zeros(srch_len,2);
rdarr_2 = zeros(srch_len,2);
psd_data_1 = zeros(num_of_points/2+1,num_of_columns_data);
psd_data_2 = zeros(num_of_points/2+1,num_of_columns_data);
...
% psd for data
for i = 1:col_opts
    [psd_data_1(:,i),~] = cpsd(filt_input_u(1,:),filt_output_x(:,i),win_opts,[],num_of_points,fs);
    [psd_data_2(:,i),~] = cpsd(filt_input_u(2,:),filt_output_x(:,i),win_opts,[],num_of_points,fs);
end
% convert to real positive estf
estf_data_1 = (psd_data_1./psd_u_1);
estf_data_2 = (psd_data_2./psd_u_2);
    ...
    case 'manul'
        ...
        switch ans_select
            ...
            case 'Go select'               % select intervals
                figure;
                plot(log10(f),log10(abs(estf_data_1(:,1))));
                grid on;
...
estf_data_in_1 = estf_data_1(interval_start:interval_end,:);
estf_data_in_2 = estf_data_2(interval_start:interval_end,:);
% plot(faxis,log10(abs((estf_data_in_1(:,3)))));
...
parfor ii = 1:srch_len
        coeff = [1;coeff_reg(1);srch_list(ii)];
        estf_feed_data = log10(abs(estf_data_in_1 * coeff));
        [rd,ptg] = getrd(dim,estf_feed_data,faxis,min_num_seg,plot_opts,coeff(2));
        rdarr_1(ii,:) =[rd ptg];
end

parfor ii = 1:srch_len
        coeff = [1;coeff_reg(2);srch_list(ii)]
        estf_feed_data = log10(abs(estf_data_in_2 * coeff));
        [rd,ptg] = getrd(dim,estf_feed_data,faxis,min_num_seg,plot_opts,coeff(2));
        rdarr_2(ii,:) =[rd ptg];
end
